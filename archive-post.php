...
<body <?php body_class(); ?>>
	...
	<main>
		<?php get_queried_object()->labels->name = "ブログ" ?>
		<?php get_template_part('template-parts/top'); ?>

		<div class="blog-container">
			<h2 class="blog__title">新着一覧</h2>
			<ul class="blog__list">
				...
				<?php while (have_posts()) : the_post(); ?>
					<li class="blog__item">
						...
						<div class="blog__item__text">
							...
							<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
								<?php
								$count_excerpt = get_the_excerpt();

								if (mb_strlen($count_excerpt) > 30) {
									$excerpt = mb_substr($count_excerpt, 0, 30);
									echo $excerpt . '...';
								} else {
									echo $count_excerpt;
								}
